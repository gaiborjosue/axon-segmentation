...
from .axon_subset_dataset import AxonSubsetDataset, collapse_labels
...
def build_gpu_tensor_cache(
    ...
    *,
    ...
    for sample_index in range(n_total):
        ...
        if dataset.segmentation_mode == 'binary':
            synth_label_t = collapse_labels(label_t, n_groups=dataset.n_label_groups)
        else:
            synth_label_t = label_t

        label_gpu = synth_label_t.to(device, non_blocking=True)
        prob_gpu = prob_t.to(device, non_blocking=True)
        target_gpu = label_t.to(device, non_blocking=True) if dataset.segmentation_mode != 'binary' else None

        t_synth = time.time()
        with torch.no_grad():
            if dataset.segmentation_mode == 'binary':
                image, out_prob = synth(label_gpu, prob_gpu)
                seg_label_t = label_t
