...
# Submit CPU threshold-baseline jobs for every annotated LSM patch.
#
# Usage:
#   bash run/production/submit_lsm_baseline_all.sh
#   DRY_RUN=1 bash run/production/submit_lsm_baseline_all.sh

DATA_ROOT="${DATA_ROOT:-/home/egaibor/orcd/scratch/LSM_axonal_marker_annotated_patches}"
SBATCH_SCRIPT="${SBATCH_SCRIPT:-run/production/baseline_lsm_annotated_patch.sbatch}"

BASELINE_KIND="${BASELINE_KIND:-threshold}"
...
for patch in "${raw_patches[@]}"; do
    ...
    export_arg="ALL,PATCH=${patch},BASELINE_KIND=${BASELINE_KIND},EVAL_TAG=${EVAL_TAG},TOPOLOGY_METRICS=${TOPOLOGY_METRICS},BEST_THRESHOLD_CRITERION=${BEST_THRESHOLD_CRITERION},NORM_MODE=${NORM_MODE}"
    if [[ "${DRY_RUN:-0}" == "1" ]]; then
        echo "sbatch --export=${export_arg} ${SBATCH_SCRIPT}"
    else
        job_id=$(sbatch --parsable --export="${export_arg}" "${SBATCH_SCRIPT}")
        echo "Submitted ${job_id}: ${patch}"
    fi
    submitted=$((submitted + 1))
done

echo "Prepared ${submitted} baseline job submissions."
